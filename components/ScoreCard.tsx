...
export default function ScoreCard({calculatedOverallScore, metrics }: ScoreCardProps) {
  ...
  return (
    <div className="flex justify-center w-full">
      <div className="flex flex-col md:flex-row bg-white p-2 rounded-md rounded-xl w-full max-w-4xl">
        {/* Overall Score */}
        <div className="flex flex-col items-center justify-center bg-green-700 text-white p-2 rounded-xl w-full md:w-1/4 mb-3 md:mb-0">
        ...
        <div className="flex-1 md:pl-6 space-y-2 w-full">
          {metrics.map((metric) => (
            <div key={metric.name} className="flex items-center justify-between">
              <div className="w-1/2 text-md text-gray-700 text-right">{metric.name}</div>
