...
export function processMarkdownContent(content: string): string {
  try {
    ...
    styledHtml = styledHtml.replace(
      /<h3>(.*?)<\/h3>/g,
      '<div class="mb-4 mt-8" style="padding-left:1rem;"><h3 class="text-lg font-semibold uppercase tracking-wide w-full inline-block" style="border-bottom:2px solid #11abea;letter-spacing:0.06em;"><span style="background-color:#11abea;color:#fff;padding:0.25rem 1rem;border-radius:0.2rem;">$1</span></h3></div>'
    );
